""" the main CLI for calling app_skel """
...
def parse_params(args):
    ...
    keys = list(app_skel.Core('null', quiet=True).paramsdict.keys())
...
def getmodel(args, params):
    ...
    project_dir = app_skel.util._expander(params['project_dir'])
    ...
    data = app_skel.Core(sim_name, quiet=args.quiet, verbose=args.verbose)
...
def parse_command_line():
  ...
  * Example command-line usage: 
    app_skel -n data                       ## create new file called params-data.txt 
    app_skel -p params-data.txt            ## run app_skel with settings in params file
    """)
    ...
    parser.add_argument('-d', action='store_true', dest="debug",
        help="print lots more info to app_skel_log.txt.")

    parser.add_argument('-V', action='version', 
        version=str(pkg_resources.get_distribution('app_skel')),
...
def do_sims(data, args):
    ...
    try:
    ...
    except app_skelError as inst:
        LOGGER.error("app_skelError: %s", inst)
        print("\n  Encountered an error (see details in ./app_skel_log.txt)"+\
              "\n  Error summary is below -------------------------------"+\
              "\n{}".format(inst))
    except Exception as inst:
        LOGGER.error(inst)
        print("\n  Encountered an unexpected error (see ./app_skel_log.txt)"+\
        ...
        try:
            ...
            if ipyclient:
                ...
                if 'app_skel-cli' in data._ipcluster["cluster_id"]:
...
def main():
    """ main function """
    app_skel.__interactive__ = 0  ## Turn off API output

    ## parse params file input (returns to stdout if --help or --version)
    args = parse_command_line()

    if not args.quiet: print(app_skel_HEADER)

    ## Turn the debug output written to app_skel_log.txt up to 11!
    ## Clean up the old one first, it's cleaner to do this here than
    ## at the end (exceptions, etc)
    if os.path.exists(app_skel.__debugflag__):
        os.remove(app_skel.__debugflag__)

    if args.debug:
        if not args.quiet: print("\n  ** Enabling debug mode **\n")
        app_skel._debug_on()
        atexit.register(app_skel._debug_off)

    ## create new paramsfile if -n
    if args.new:
        ## Create a tmp assembly, call write_params to make default params.txt
        try:
            tmpassembly = app_skel.Core(args.new, quiet=True)
    ...
    with open(app_skel.__debugfile__, 'a') as logfile:
        logfile.write(app_skel_HEADER)
    ...
    if args.params:
        ...
        if args.sims:
            ...
            if os.path.exists(app_skel.__debugfile__):
                if os.path.getsize(app_skel.__debugfile__) > 50000000:
                    with open(app_skel.__debugfile__, 'w') as clear:
...
app_skel_HEADER = \
"\n -------------------------------------------------------------"+\
"\n  app_skel [v.{}]".format(app_skel.__version__)+\
...
app_skel_USAGE = """
    ...
    e.g., app_skel -p params-test.txt -s 10000           ## run 10000 simulations
    """
