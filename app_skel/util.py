...
class app_skelError(Exception):
    """ General app_skel exception """
...
def tuplecheck(newvalue, dtype=str):
    ...
    if isinstance(newvalue, tuple):
        ...
        try:
            ...
            raise app_skelError("tuplecheck failed for {}, improper list format".format(newvalue))
    else:
        try:
            ...
            try:
                ...
                raise app_skelError("{}\ttuplecheck() failed to cast to {} - {}"\
...
def set_params(data, param, newvalue, quiet=True):
    ...
    if not param in list(data.paramsdict.keys()):
        raise app_skelError("Parameter key not recognized: {}"\
                                .format(param))
    try:
        data._paramschecker(param, newvalue, quiet)
    except Exception as inst:
        raise app_skelError(BAD_PARAMETER.format(param, inst, newvalue))
    return data
...
def _load_sims(sims, sep=" "):
    ...
    if isinstance(sims, str):
        ...
        raise app_skelError("Input simulations not understood. Must be a file name or a pandas DataFrame")
    return sim_df
